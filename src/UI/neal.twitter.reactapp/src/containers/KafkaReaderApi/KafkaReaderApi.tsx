...
/**
 * Represents the information provided by the Kafka Tweet API.
 * @returns
 */
function KafkaReaderApi() {
    ...
    const GetData = () => {
        requestData('http://localhost:4200/api/tweets/hashtags')
            .then((result: unknown) => {
                if (result !== undefined) {
                    setHashtags(result as Hashtag[]);
                }
            });
        requestData('http://localhost:4200/api/tweets/count')
            .then((result: unknown) => {
                if (result !== undefined) {
